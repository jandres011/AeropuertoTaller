...
class AereolineaRepositoryTest {
    ...
    void setUp() {
        ...
        aereolinea1 = Aereolinea.builder().nombre("Jetsmart").vuelos(vuelos).build();
    ...
    void findAereolineaByNombreStartingWith() {
        List<Aereolinea> aereolineasTest = aereolineaRepository.findAereolineaByNombreStartingWith("J");
        assertFalse(aereolineasTest.isEmpty());
        assertEquals(1, aereolineasTest.size());
        assertEquals("Jetsmart", aereolineasTest.get(0).getNombre());
    }
    ...
    void findAereolineaByIdAereolinea() {
        ...
        assertEquals("Jetsmart", aereolineaTest.getNombre());
    }
    ...
    void findAereolineasByNombreLike() {
        List<Aereolinea> aereolineasTest = aereolineaRepository.findAereolineasByNombreLike("%Air%");
        assertFalse(aereolineasTest.isEmpty());
        assertEquals(1, aereolineasTest.size());
    ...
    void getAirlinesByDestination() {
        List<Aereolinea> aereolineasTest = aereolineaRepository.getAirlinesByDestination("Santa Marta");
        assertFalse(aereolineasTest.isEmpty());
        assertEquals(1, aereolineasTest.size());
        assertEquals("Jetsmart", aereolineasTest.get(0).getNombre());
    }
